...
template<class T> inline Ptr::unique<T>::~unique() {
    delete ptr;
}
...
template<class T> inline Ptr::shared<T>::shared(const Ptr::shared<T> &p) 
...
template<class T> inline Ptr::shared<T> &Ptr::shared<T>::operator=(const Ptr::shared<T> &p) {
